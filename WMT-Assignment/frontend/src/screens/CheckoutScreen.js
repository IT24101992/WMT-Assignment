...
import { createOrder, removeSelectedFromCart, fetchMyOrders } from '../services/api';
...
export default function CheckoutScreen({ route, navigation }) {
  ...
  // Load user orders (READ operation)
  ...
  // Enhanced validation with error display
  const validateForm = () => {
  ...
  // Enhanced place order function (CREATE operation)
  ...
  // Render order item for orders modal
  const renderOrderItem = ({ item }) => (
    <View style={styles.orderItem}>
      <View style={styles.orderItemHeader}>
        <Text style={styles.orderId}>Order ID: {item._id.slice(-8).toUpperCase()}</Text>
        <Text style={[styles.orderStatus, styles[`status${item.status.replace(/\s+/g, '')}`]]}>
          {item.status}
        </Text>
      </View>
      ...
      <Text style={styles.orderTotal}>LKR {item.totalPrice.toLocaleString()}</Text>
      <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  paymentOptionSelected: {
    borderColor: '#BFA46A', backgroundColor: '#FBFAF7',
  },
  radioBtn: {
  ...
  radioBtnSelected: {
    borderColor: '#BFA46A', backgroundColor: '#BFA46A',
  },
  paymentOptionContent: {
    flex: 1,
  },
  paymentOptionTitle: {
    fontSize: 14, fontWeight: '700', color: '#1B1B1B', marginBottom: 2,
  },
  paymentOptionDesc: {
    fontSize: 12, color: '#8A8175',
  },
  footer: {
  ...
  loadingText: {
    marginTop: 10, fontSize: 16, color: '#8A8175',
  },
  emptyContainer: {
    flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20,
  },
  emptyText: {
    fontSize: 18, fontWeight: '700', color: '#1B1B1B', marginBottom: 8,
  },
  emptySubText: {
    fontSize: 14, color: '#8A8175', textAlign: 'center',
  },
  ordersList: {
    padding: 16,
  },
  ...
  orderDate: {
    fontSize: 12, color: '#8A8175', marginBottom: 4,
  },
  orderTotal: {
    fontSize: 16, fontWeight: '800', color: '#BFA46A', marginBottom: 12,
  },
  viewOrderBtn: {
    backgroundColor: '#BFA46A', borderRadius: 8, padding: 10, alignItems: 'center',
  },
  viewOrderBtnText: {
